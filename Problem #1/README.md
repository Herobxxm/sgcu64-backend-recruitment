...
## For user(์ Note สำหรับผู้ใช้งาน หรือ ใครก็ตามที่อ่านแล้วมาเจอตรงนี้ )

- แนะนำให้ run file ผ่าน .exe เพราะการ run ผ่าน ide เช่น vsCode อาจทำให้เกิด Bug หรือ Console แสดง Output ที่ไม่เหมาะสมได้
- ได้โปรดใช้ตัว Program ตาม instruction ด้วย(เพราะไม่ได้เขียนดักไว้อะ)

## instruction

1. เมื่อเริ่ม Timer.exe จะมี ข้อความเด้งว่า
         input: 
   ให้ใส่ข้อความตามแบบฟอร์มดังนี้ เมื่อให้ X แทนด้วย ตัวเลข 0 - 9
         XX:XX:XX
2. จะมี Output เป็นรูปแบบ Seven Segment แสดงออกมาตามที่โจทย์บอก
3. (Optional) ถ้าหากเราส่งเวลาอย่างถูงต้อง จะมีคำถาม ถามว่าให้นับเวลาถอยหลังให้ไหม
     ใส่ y เพื่อนับถอยหลัง
     ใส่ n เพื่อออกProgram
    *** ตัวพิมพ์ใหญ่ พิมพ์เล็บ ก็ได้***
